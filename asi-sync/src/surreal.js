...
async function mergeRecord(db, id, data) {
  const rid = asRecord(id);
  if (typeof db.update === 'function') {
    await db.update(rid).merge(data);
    return;
  }
  if (typeof db.merge === 'function') {
    await db.merge(rid, data);
    return;
  }
  await queryRows(db, `UPDATE $id MERGE $data`, { id: rid, data });
}
